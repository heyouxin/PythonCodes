...
##取hs300成分股及权重数据，取股票月度数据。取出hs300中的股票，并按市值、账面市值比分组
##2016年6月30日指数权重数据没有，暂用2017年6月30日的数据代替
    def size_bm_group(self,breakyear):
    
        global hs300s_stock_m,hs300s_stock_breakpoint,stock_SH,stock_SM,stock_SL,stock_BH,stock_BM,stock_BL
        #string1='C:/Users/heyouxin/Desktop/HS300数据/hs300_weights_'
        #string2='.xls'
        #hs_file="%s%s%s"%(string1,breakyear,string2)
        hs_file='./HS300数据/hs300_weights_'+breakyear+'.xls'
        hs300s=pd.read_excel(hs_file)
        #stock_file="%s%s%s"%('C:/Users/heyouxin/Desktop/HS300数据/stock_',breakyear,'.xls')
        stock_file='./HS300数据/stock_'+breakyear+'.xls'
    ...
    def calc_factors(self):
        ...
        for i in range(0,12):
            ...
            self.size_bm_group(breakpoint[i])
            SMB_temp=self.calc_SMB(breakpoint[i],breakpoint[i+1])
            SMB=SMB+SMB_temp
            HML_temp=self.calc_HML(breakpoint[i],breakpoint[i+1])
            HML=HML+HML_temp
            R_25_group_temp=self.calc_R_Rf(breakpoint[i],breakpoint[i+1])
            R_25_group=R_25_group.append(R_25_group_temp)
            UMD_temp=self.calc_UMD(breakpoint[i],breakpoint[i+1])
            UMD=UMD.append(UMD_temp)
        SMB=np.array(SMB)
        HML=np.array(HML)
        Rm_Rf=np.array(self.calc_RM_Rf())
        UMD=np.array(UMD)
        return (R_25_group,SMB,HML,Rm_Rf,UMD)
            
        
    ##计算SMB因子
    def calc_SMB(self,breakyear,breakyear1):
    ...
    ##计算HML因子  
    def calc_HML(self,breakyear,breakyear1):
    ...
    def calc_R_Rf(self,breakyear,breakyear1):
    ...
    ##CAPM模型回归
    def do_regression_CAPM(self,R_25_group,Rm_Rf):
    ...
    ##两因子回归
    def do_regression_2factors(self,R_25_group,Rm_Rf,HML):
        #(R_25_group,SMB,HML,Rm_Rf,UMD)=calc_factors()
        alpha1=[]
        t_alpha1=[]
        belta_mkt=[] 
    ...
    ##三因子回归
    def do_regression_3factors(self,R_25_group,Rm_Rf,HML,SMB):
        #(R_25_group,SMB,HML,Rm_Rf,UMD)=calc_factors()
        alpha1=[]
        t_alpha1=[]
        belta_mkt=[] 
    ...
    ##四因子回归
    def do_regression_4factors(self,R_25_group,Rm_Rf,HML,SMB,UMD):
        #(R_25_group,SMB,HML,Rm_Rf,UMD)=calc_factors()
        alpha1=[]
        t_alpha1=[]
        belta_mkt=[] 
    ...
    def display(self,R_25_group,SMB,HML,Rm_Rf,UMD):
        ...
        print('-------------------------')
        print('CAPM model')
        (alpha_CAPM,t_alpha_CAPM,belta_mkt_CAPM,R2_CAPM)=self.do_regression_CAPM(R_25_group,Rm_Rf)
        print(alpha_CAPM)
        print(t_alpha_CAPM)
        ...
        print('-------------------------')
        print('2factors model')
        (alpha_2factors,t_alpha_2factors,belta_mkt_2factors,belta_hml_2factors,R2_2factors)=self.do_regression_2factors(R_25_group,Rm_Rf,HML)
        print(alpha_2factors)
        print(belta_mkt_2factors)
        print(belta_hml_2factors)
        print(R2_2factors)
        
        print('-------------------------')
        print('3factors model')
        (alpha_3factors,t_alpha_3factors,belta_mkt_3factors,belta_hml_3factors,belta_smb_3factors,R2_3factors)=self.do_regression_3factors(R_25_group,Rm_Rf,HML,SMB)
        print(alpha_3factors)
        print(t_alpha_3factors)
        print(belta_mkt_3factors)
        print(belta_hml_3factors)
        print(belta_smb_3factors)
        print(R2_3factors)
    
        print('-------------------------')
        print('4factors model')
        (alpha_4factors,t_alpha_4factors,belta_mkt_4factors,belta_hml_4factors,belta_smb_4factors,belta_umd_4factors,R2_4factors)=self.do_regression_4factors(R_25_group,Rm_Rf,HML,SMB,UMD)
        print(alpha_4factors)
        print(t_alpha_4factors)
        print(belta_mkt_4factors)
        print(belta_hml_4factors)
        print(belta_smb_4factors)
        print(belta_umd_4factors)
        print(R2_4factors)
