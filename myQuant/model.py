...
class Model:
    ...
    hs300s_stock_m=DataFrame()
    hs300s_stock_breakpoint=DataFrame()
    stock_SH=DataFrame()
    stock_SM=DataFrame()
    stock_SL=DataFrame()
    stock_BH=DataFrame()
...
##2016年6月30日指数权重数据没有，暂用2017年6月30日的数据代替
    def size_bm_group(self):
        ...
        self.hs300s_stock_m=hs300s_stock_m
        ##取出调整日当天的hs300数据
        ...
        self.hs300s_stock_breakpoint=hs300s_stock_breakpoint
        ...
        ##HML = (SH+BH)/2-(SL+BL)/2
        ##获得分组股票代码
        stock_SH=DataFrame(list(set(hs300s_stock_size_small).intersection(set(hs300s_stock_bm_high))))
        stock_SM=DataFrame(list(set(hs300s_stock_size_small).intersection(set(hs300s_stock_bm_middle))))
        stock_SL=DataFrame(list(set(hs300s_stock_size_small).intersection(set(hs300s_stock_bm_low))))
        
        stock_BH=DataFrame(list(set(hs300s_stock_size_big).intersection(set(hs300s_stock_bm_high))))
        stock_BM=DataFrame(list(set(hs300s_stock_size_big).intersection(set(hs300s_stock_bm_middle))))
        stock_BL=DataFrame(list(set(hs300s_stock_size_big).intersection(set(hs300s_stock_bm_low))))
        ...
        self.stock_SH=stock_SH
        self.stock_SM=stock_SM
        self.stock_SL=stock_SL
    ...
    def calc_SMB(self,breakyear,breakyear1):
        ...
        stock_SM=self.stock_SM
        stock_SL=self.stock_SL
        stock_BH=self.stock_BH
        stock_BM=self.stock_BM
        stock_BL=self.stock_BL
