...
def Count_issuer_history(df, number, all_cat, time_now = ''):
...
def Count_history_underwirter(df, codee, all_cat, under_dict_code, time_now=''):
    if time_now == '':
...
def Create_variable_underwriter(Bond, all_cat, time_now):
    ...
    for codee in Bond.code:
        cc = Count_history_underwirter(Bond, codee, all_cat, under_dict_code, time_now)
...
def Revise_variables(df, all_cat, time_now):
    ...
    df = Create_variable_underwriter(df, all_cat, time_now)
...
def Data_preparation(already, default, time_now):
    ...
    test.date_end = time_now
    test = Calculate_mean(test, 'date_end')
...
def DataSplit(split_date):

    print('Reading raw data.')
    Already = pd.read_excel("data/Already_quarter_0913.xlsx")
    Default = pd.read_excel("data/Default_quarter_0913.xlsx")
    #Already = pd.read_excel("already_bond_0903.xlsx")
    #Default = pd.read_excel("default_bond_0903.xlsx")
    global T_start, number_start, number_end, time_default, all_cat
    # noinspection PyRedeclaration
    #T_start = pd.Timestamp('20180701')
    T_start = pd.Timestamp(split_date)
