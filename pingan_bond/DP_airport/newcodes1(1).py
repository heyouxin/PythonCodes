...
#参数
P = pd.DataFrame({'1':[3,2,0],'2':[5,6,0],'3':[2,4,0],'4':[4,3,0],'5':[5,3,0]}) #包含了分配给dummy gate 的效用
...
G_matrix=np.ones([num,num])*(-1000)
row=0
for i in range(0,len(D)):
    for j in range(0,len(D[i])):

        for k in range(0,len(D[i][j])):
            G_matrix[row,row+k+1]=D[i][j][k]
        row = row + 1


G_matrix=G_matrix*(-1)
G_matrix[G_matrix<0]=100
import networkx as nx
G_nx = nx.DiGraph(G_matrix)

nx.bellman_ford_path(G_nx,0,5)
...
path=nx.dijkstra_path(G_nx, source=0, target=6)
print('节点0到7的路径：', path)
print('dijkstra方法寻找最短距离：')
distance=nx.dijkstra_path_length(G_nx, source=0, target=14)
print('节点0到7的距离为：', distance)

pos=nx.shell_layout(G_nx)
print('画出网络图像：')
nx.draw(G_nx,pos,with_labels=True, node_color='white', edge_color='red', node_size=400, alpha=0.5 )
